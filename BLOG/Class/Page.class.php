...
class Page {
    ...
    public function __construct($totalRows,$listRows='',$parameter='',$url='') {
        ...
        if(preg_match("/page_(\d+)$/", $_SERVER['REQUEST_URI'], $matches)){
            $this->nowPage = explode('_',$matches[0])[count($matches)-1];
        }
    ...
    public function show() {
        ...
        $suffixUrl = $_SERVER['REQUEST_URI'];
        $suffixUrl = preg_replace("/page_(\d+)/", "", $suffixUrl);
        $suffixUrl = substr($suffixUrl, 1);
        $initUrl = 'http://'.$_SERVER['SERVER_NAME'];//.$_SERVER['REQUEST_URI']
        
        $initUrl = preg_replace("/page_(\d+)/", "", $initUrl);
        ...
        if($this->url){
            ...
            //$url            =   U('',$parameter);
            $url            =   $initUrl.'/page_'.'__PAGE__'.$suffixUrl;
        }
